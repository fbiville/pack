...
type Stack struct {
	...
	BuildImages []string `toml:"build-images,omitempty"`
...
func (c *Config) Add(stack Stack) error {
	if _, err := c.Get(stack.ID); err == nil {
		return fmt.Errorf(`stack %s already exists`, style.Emphasized(stack.ID))
	}
...
func (c *Config) Delete(stackID string) error {
	if c.DefaultStackID == stackID {
		return fmt.Errorf(`stack %s cannot be deleted when set as the default stack -- you can change your default stack by running %s`, style.Emphasized(stackID), style.Emphasized("pack set-default-stack"))
	}
...
func missingStackError(stackID string) error {
	return fmt.Errorf(`stack %s does not exist`, style.Emphasized(stackID))
}
